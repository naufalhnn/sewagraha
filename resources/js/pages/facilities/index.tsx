          ...
          <table className="w-full table-auto rounded-md">
            ...
            <tbody>
              {facilities.map((facility) => (
                <tr key={facility.id}>
                  <td className="border-1 px-4 py-1">{facility.name}</td>
                  <td className="border-1 px-4 py-1">{facility.description}</td>
                  <td className="flex justify-center space-x-1 border px-4 py-1 text-center">
                    ...
                    <Dialog open={open && selectedFacility?.id === facility.id} onOpenChange={setOpen}>
                      ...
                      <DialogContent>
                        ...
                        <DialogFooter>
                          ...
                          <Button variant={'destructive'} onClick={handleDelete} className="cursor-pointer transition duration-300 hover:bg-red-700">
                            Hapus
                          </Button>
                        </DialogFooter>
                      </DialogContent>
